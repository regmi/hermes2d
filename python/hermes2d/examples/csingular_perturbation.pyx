from hermes2d._hermes2d cimport scalar, FuncReal, GeomReal, WeakForm, \
        int_grad_u_grad_v, int_grad_u_grad_v_ord, int_v, int_u_v, int_v_ord, \
        malloc, ExtDataReal, c_Ord, create_Ord, FuncOrd, GeomOrd, \
        ExtDataOrd, int_u_v, int_u_v_ord, BC_NATURAL, H2D_SYM, \
        BC_ESSENTIAL, c_BCType, H1Space, c_atan, c_pi, c_sqrt, c_sqr, \
        c_atan2, c_cos

# Problem parameters.
cdef double K_squared = 1e4    
...
cdef scalar bilinear_form(int n, double *wt, FuncReal **t, FuncReal *u, FuncReal *v, GeomReal
        *e, ExtDataReal *ext):
    return int_grad_u_grad_v(n, wt, u, v) + K_squared * int_u_v(n, wt, u, v)

cdef scalar linear_form(int n, double *wt, FuncReal **t, FuncReal *v, GeomReal
        *e, ExtDataReal *ext):
    return K_squared * int_v(n, wt, v)
